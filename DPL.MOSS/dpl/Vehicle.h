...
public:
	...
	void GetPosition(Vector4* returnStorage);
	void GetOrientation(Vector4* returnStorage);
	...
	Vector4* GetVelocity(Vector4* returnStorage);
	Vector4 GetVelocity();
	...
	void SetDirection(Vector4* quat);
	...
	static void __fastcall Hook_ManipulationPacketSent_Thunk(CVehicle* self, SVehicleManipulationPacket* packet) {
		// unsafe and crashes the game ...
		/*
		static int oldESP = NULL;
		static int oldEBP = NULL;
		...
		*/
		...
		if (self != NULL)
		{
			if (packet != NULL)
			{
				...
				self->Hook_ManipulationPacketSent(packet);
				self->SendManipulationPacketInternal(packet);
			}
		}
	...
	void Hook_ManipulationPacketSent(SVehicleManipulationPacket* packet);
	static void Hook_VehicleCtor();
	static void Hook_VehicleDtor();

	static void Hook_VehicleCreate();
